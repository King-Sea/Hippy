...
- (void)runDemoWithoutRuntime {
    ...
    _domManager->CreateDomNodes(_rootNode, std::move(nodesData));
    
    //6.end batch
    _domManager->EndBatch(_rootNode);
}
...
- (void)initRenderContextWithRootView:(UIView *)rootView {
    ...
    if (rootView && hippyTag) {
        ...
        _domManager->SetRootSize(_rootNode, CGRectGetWidth(rootView.bounds), CGRectGetHeight(rootView.bounds));
...
- (std::vector<std::shared_ptr<hippy::DomNode>>) mockNodesData {
    ...
    for (NSDictionary<NSString *, id> *mockNode in mockJson) {
        ...
        std::shared_ptr<hippy::DomNode> dom_node = std::make_shared<hippy::DomNode>(tag, pid, index, name, name, std::move(style_map), std::move(ext_map), _rootNode);
